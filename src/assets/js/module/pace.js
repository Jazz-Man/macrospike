  ...
  for (_i = 0, _len = sources.length; _i < _len; _i++) {
    ...
    if (source) {
      for (key in source) {
        ...
        val = source[key];
        if ((
            out[key] != null) && typeof out[key] === 'object' && (
...
Evented = (
  function () {
    ...
    Evented.prototype.on = function (event, handler, ctx, once) {
      ...
      if ((
          _base = this.bindings)[event] == null) {
        _base[event] = [];
      }
...
extendNative = function (to, from) {
  ...
  for (key in from.prototype) {
    try {
      if ((
          to[key] == null) && typeof from[key] !== 'function') {
    ...
    if (!Pace.running && (
      ...
      return setTimeout(function () {
        ...
        if (type === 'socket') {
          ...
          stillActive = (
            0 < (
              _ref2 = request.readyState) && _ref2 < 4);
        }
        if (stillActive) {
          Pace.restart();
          _ref3 = Pace.sources;
      ...
      if (window.ProgressEvent != null) {
        ...
        request.addEventListener('progress', function (evt) {
          if (evt.lengthComputable) {
            ...
            return _this.progress = _this.progress + (
    ...
    Scaler.prototype.tick = function (frameTime, val) {
      ...
      if (val === this.last) {
        ...
        if (this.sinceLastUpdate) {
          this.rate = (
            val - this.last) / this.sinceLastUpdate;
        }
        this.catchup = (
          val - this.progress) / options.catchupTime;
...
(
  init = function () {
    ...
    _ref4 = (
      _ref3 = options.extraSources) != null ? _ref3 : [];
...
Pace.go = function () {
  ...
  return animation = runAnimation(function (frameTime, enqueueNextFrame) {
    ...
    if (bar.done() || done || cancelAnimation) {
      ...
      }, Math.max(options.ghostTime, Math.max(options.minTime - (
        now() - start), 0)));
    } else {
